...
function generateKeys(phrase) {
  ...
  if (match) {
    ...
    return [match.hint.map((word) => word.includes("[") ? word.split(" ").map((value, index) => value.at(0) === "[" ? value.replace(`[${words[index].pos}]`, words[index].simple).join(" ") : value) : word), submeaning];
  }
