...
handlebars.registerHelper('hover-translate', function(arg, lang, options) {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (lang === "en" && Object.keys(dictionary.smb).filter(key => dictionary.smb[key].complex.includes(save + tokens[i + 1])).length === 0) {
      keys = Object.keys(dictionary.smb).filter(key => dictionary.smb[key].complex.includes(save.trimEnd()));
